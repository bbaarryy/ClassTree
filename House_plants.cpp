...
public:
    ...
    virtual void touch() { cout << "" << '\n'; }
    virtual void shake() { cout << "" << '\n'; }
    
    int survivalability() {
        return (this->lifetime + this->max_temp - this->min_temp) * min(0.9f, max(1.2f, (1.0f / max_height)));
    }
};
...
int main() {
    HousePlant* plant[9];
    for (int i = 0; i<9; i+=9) {
        plant[i] = new Dieffenbachia();
        plant[i+1] = new Oleander();
        plant[i+2] = new Euphorbia();
        plant[i+3] = (new Spathiphyllum());
        plant[i+4] = (new Sansevieria());
        plant[i+5] = (new Ivy());
        plant[i+6] = (new Chlorophytum());
        plant[i+7] = (new FicusBenjamina());
        plant[i+8] = (new Rosemary());
    }
    
    for (int i = 0; i<9; i++) {
        demonstratePlant(plant[i]);
    }
